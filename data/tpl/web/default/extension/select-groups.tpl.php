...
<div class="clearfix">
	...
	<div class="alert alert-info">
		您正在安装 <?php  if($action == 'module') { ?><?php  echo $module['title'];?> 模块<?php  } else { ?>模板<?php  } ?>. 请选择哪些项目服务套餐组可使用 
	...
	<form class="form-horizontal form" action="" method="post" id="form1">
		<h5 class="page-header">可用的项目服务套餐组 <small>这里来定义哪些项目服务套餐组可使用 <?php  if($action == 'module') { ?><?php  echo $module['title'];?> 功能<?php  } else { ?>该模板<?php  } ?></small></h5>
		<div class="form-group">
			<label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">项目服务套餐组</label>
	...
	<script>
		$('#form1').submit(function(){
			...
			if(num == 0) {
				return confirm("您没有选择可使用该模块/模板的项目服务套餐组,模块/模板安装成功后可在 项目服务套餐 编辑");
			}
			return true;
		});
	</script>
