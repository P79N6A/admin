...
<ul class='order-list'>
    ...
    <?php  } ?>
    <li <?php  if($_W['routes']=='order.list.status6') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('order/list/status6')?>">未审核 <span class='status6'>--</span></a>
    </li>
    <li <?php  if($_W['routes']=='order.list.status7') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('order/list/status7')?>">已通过 <span class='status7'>--</span></a>
    </li>
    <li <?php  if($_W['routes']=='order.list.status8') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('order/list/status8')?>">未通过 <span class='status8'>--</span></a>
    </li>
</ul>
<?php  } ?>
<?php  } ?>
...
<ul class='order-list'>
    ...
    <li <?php  if($_W['routes']=='order.list.status4') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('order/list/status4')?>">维权申请 <span class='status4'>--</span></a>
    </li>
    <?php  } ?>
...
<script>
    $(function () {
        $.ajax({type: "GET",url: "<?php  echo webUrl('order/list/ajaxgettotals')?>",dataType:"json",success: function(data){
                ...
                $("span.status6").text(res.status6);
                $("span.status7").text(res.status7);
                $("span.status8").text(res.status8);
