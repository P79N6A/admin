...
<ul>
    <?php if(cv('store.verifylog')) { ?><li <?php  if($_W['action'] == 'store.verify') { ?> class="active" <?php  } ?>><a href="<?php  echo webUrl('store/verify/log')?>">记次时商品记录</a></li><?php  } ?>
    <?php if(cv('store.verifyorderlog')) { ?><li <?php  if($_W['action'] == 'store.verifyorder') { ?> class="active" <?php  } ?>><a href="<?php  echo webUrl('store/verifyorder/log')?>">核销订单记录</a></li><?php  } ?>
</ul>
<?php  } ?>
