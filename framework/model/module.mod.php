...
function module_node_permission(){
    global $_GPC;
    $module_name = $_GPC['m'];
    $current_module_permission = module_permission_fetch($module_name);
    $module = array();
    foreach ($current_module_permission as $cur){
        if(stripos($cur['permission'],'menu')!==false){
            $item = explode('_menu_',$cur['permission']);
            $module[] = $item[1];
        }
    }

    $dos = implode("','",$module);
    $menus = pdo_fetchall("select eid,title from ims_modules_bindings  where module=:module and entry='menu' and do in ('".$dos."')",array(':module'=>$module_name));
    $menu_id = array();
    foreach ($menus as $ls){
        $menu_id[] = $ls['eid'];
    }
    $menuStr = implode(",",$menu_id);
    $list2 = pdo_fetchall("select * from ims_modules_nodes  where menu_id in (".$menuStr.")");
    $arr = array();
    foreach ($menus as $ls1){
        foreach ($list2 as $ls2){
            if($ls1['eid']==$ls2['menu_id']){
                $arr[$ls1['eid']]['title'] = $ls1['title'];
                $arr[$ls1['eid']]['son'][] = $ls2;
            }
        }
    }
